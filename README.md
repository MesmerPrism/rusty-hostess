...
The opt-in complete-product Windows alpha distribution is documented in
[docs/WINDOWS_ALPHA_DISTRIBUTION.md](docs/WINDOWS_ALPHA_DISTRIBUTION.md). It
...
- `tools/hostessctl/hostessctl.py companion-report operator-actions`: emits a
  read-only `rusty.hostess.companion.operator_action_catalog.v1` report for
  WPF-visible operator actions. It is the automation-facing catalog for action
  ids, WPF command properties, PowerShell-shaped Hostess CLI recipes, evidence
  artifacts, authority owners, and test coverage; it does not run the recipes
  or mutate host/headset state.
