...
option shorthand such as `QCL-010|QCL-080`. Every Hostess CLI segment in an
operator action must name a primary `--out` report artifact, and
report-producing route segments such as `connectivity-probe run` must name
their `--out` artifact explicitly. Operator-visible route text should be close
enough to automation that a future smoke can copy the same shape after filling
the placeholders.
...
When those dependencies are ready, the preferred live operator route is
`connectivity-probe qcl082-product-media-live-session`. Hostess arms the
bounded RMANVID1 TCP receiver before it sends the Quest/Manifold
`start_source` command, then writes the same request, bridge evidence, live
