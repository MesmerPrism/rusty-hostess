...
script_mod! {
    ...
    mod.draw.DrawStimulusStereoField = mod.std.set_type_default() do #(DrawStimulusStereoField::script_shader(vm)){
        ...
        volume_density: fn(p: vec3f) -> float {
            ...
            let grain = sin((p.x * 12.9898 + p.y * 78.233 + p.z * 37.719) * 1.73 + phase)
            let field = axial * 0.42 + folded * 0.33 + lattice * 0.19 + grain * 0.06
        ...
        volume_raymarch_sample: fn(surface_uv: vec2f, display_eye_selector: float, sample_t: float) -> vec4 {
            ...
            let local_light = 0.44 + 0.56 * (0.5 + 0.5 * sin((p.z + p.x * 0.45) * max(self.volume_grid_frequency, 1.0) + self.volume_phase))
