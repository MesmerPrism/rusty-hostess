...
PROTOCOL_MATRIX_CLI_ROUTE = (
    ...
    r"$PreviewFfplay = '<ffplay>'; "
    r"$PreviewWindowTitle = 'Rusty QCL-082 Camera2 direct-WiFi preview'; "
    ...
    r"python $HostessCtl connectivity-probe qcl082-product-media-plan --out $ProductMediaPlan --promoted-topology-report $PromotedTopologyReport --firewall-report $FirewallVerify --adb $Adb --serial $QuestSerial --preview-ffplay $PreviewFfplay --preview-window-title $PreviewWindowTitle --quest-lease-id $QuestLeaseId --quest-lease-resource $QuestLeaseResource; "
    r"python $HostessCtl connectivity-probe direct-wifi-product-media-plan --out $DirectWifiProductMediaPlan --qcl040-preflight-report $Qcl040Preflight --qcl041-preflight-report $Qcl041Preflight --qcl040-topology-report $Qcl040LifecycleReport --qcl041-topology-report $Qcl041LifecycleReport --promoted-topology-report $PromotedTopologyReport --firewall-report $FirewallVerify --qcl082-report $Qcl082ReceiverReport --adb $Adb --serial $QuestSerial --preview-ffplay $PreviewFfplay --preview-window-title $PreviewWindowTitle --quest-lease-id $QuestLeaseId --quest-lease-resource $QuestLeaseResource; "
    ...
    r"python $HostessCtl connectivity-probe wifi-direct-lifecycle-plan --probe-id QCL-040 --out $Qcl040LifecyclePlan --preflight-report-out $Qcl040Preflight --adb $Adb --serial $QuestSerial; "
    r"python $HostessCtl connectivity-probe wifi-direct-lifecycle-plan --probe-id QCL-041 --out $Qcl041LifecyclePlan --preflight-report-out $Qcl041Preflight --adb $Adb --serial $QuestSerial; "
    ...
    r"python $HostessCtl companion-report projection --protocol-matrix $ProtocolMatrix --include-protocol-matrix-inputs --suite-run $SuiteRun --firewall-rule $FirewallVerify --direct-wifi-product-media-plan $DirectWifiProductMediaPlan --out $Projection; "
    r"python $HostessCtl companion-report transport-gates --projection $Projection --out $TransportGates --fail-on-error --fail-on-pending --fail-on-incomplete"
)
...
def operator_actions_for_frontend(frontend: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        action(
            ...
            "else { "
            + HOSTESS_CTL
            + "connectivity-probe run --mode live --probe-id QCL-010 --out $ConnectivityReport --adb $Adb --serial $QuestSerial --tcp-echo-port $TcpEchoPort --tcp-listener-helper $HostessCompanionWpfExe }",
